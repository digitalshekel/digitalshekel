...
#if defined(NDEBUG)
# error "Digitalshekel cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x111a890e7bdb193ff89ee5dad8205f6d174a200ad7348b7806eb3fa62f4ff493");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Digitalshekel: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Digitalshekel Signed Message:\n";
...
{
    // Digitalshekel: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Digitalshekel: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Digitalshekel: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Digitalshekel: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Digitalshekel: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Digitalshekel: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x111a890e7bdb193ff89ee5dad8205f6d174a200ad7348b7806eb3fa62f4ff493");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "At Vice CuttingEdge Media and Claims of OldSchool Harassment";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("974284710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1514142138;
        ...
        assert(block.hashMerkleRoot == uint256("0x9e27ef328799c40429036db8b4b3aecd9df333c6444ed900b58e7fe4aa4ef914‏"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Digitalshekel: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DigitalshekelMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DigitalshekelMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DigitalshekelMiner(CWallet *pwallet)
{
    printf("DigitalshekelMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("digitalshekel-miner");
    ...
    try { loop {
        ...
        printf("Running DigitalshekelMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DigitalshekelMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DigitalshekelMiner, pwallet));
}
